...
function canManage(actor: object | null): boolean {
  const role = actor ? Reflect.get(actor, 'roleKey') : null
  return role === 'admin' || role === 'editor' || role === 'super-admin' || role === 'tenant-admin' || Reflect.get(actor ?? {}, 'canManagePosts') === true
}

export default definePolicy('example-sveltekit-posts', Post, {
  class: {
