...
All-matching selection keeps its captured query across table searches and pagination. Changing a bound widget or dashboard search or filter invalidates that selection when it no longer matches the bound scope. The server rejects pending bulk actions with the outdated selection; select the records again after the widget refreshes.
