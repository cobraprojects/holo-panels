...
export function NextPanelClient({ notificationRealtime, payload, tenantTransport }: NextPanelClientProps): ReactNode {
  const store = useMemo(() => {
  ...
  const notificationTrigger = notificationStore && notificationConfiguration
    ? <ReactNotificationInboxTrigger navigate={browserNavigate} panelId={state.panelId} placement={notificationConfiguration.placement} store={notificationStore} />
    : null
  ...
  return <div className="hp-panel-shell" data-panel={state.panelId} data-theme={manifest.theme.darkMode}>
    ...
    <main>
      ...
      <section data-page={payload.page.manifest.id}>{body?.component === 'resource-page'
        ? <Suspense fallback={<div aria-busy="true">Loading resource…</div>}><LazyResourcePage data={payload.page.data} effects={effects} panelId={state.panelId} panelPath={manifest.path} properties={body.properties} /></Suspense>
