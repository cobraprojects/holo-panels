...
registerPanelNotificationDispatcher(async (message, delivery) => {
  ...
  const presentation = notificationPresentation(message)
  if (delivery.kind === 'toast') {
    if (presentation.actions.some(action => action && typeof action === 'object' && !Array.isArray(action) && action.kind === 'execute')) throw new Error('Executable notification actions require database delivery')
...
export async function withPanelNotificationContext<TResult>(resolveScope: () => Promise<DeliveryScope>, operation: () => TResult | Promise<TResult>): Promise<TResult> {
  const context: NotificationContext = { active: true, effects: [], resolveScope }
