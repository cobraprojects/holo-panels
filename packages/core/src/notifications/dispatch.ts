...
type NotificationDispatcher = (message: PanelNotificationMessage, delivery: PanelNotificationDelivery) => Promise<void>
...
export async function dispatchPanelNotification(message: PanelNotificationMessage, delivery: PanelNotificationDelivery): Promise<void> {
  if (!dispatcher) throw new Error('Panel notifications require an active server panel request or a configured sender')
  await dispatcher(message, delivery)
}
