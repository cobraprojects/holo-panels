...
function ResolvedWidgetGrid({ dashboardFilters, effects, label, panelId, registry, widgets }: {
  ...
  return <ReactDashboardRenderer
    ...
    widgets={widgets.map(widget => ({
      ...
      render: () => <ResolvedWidgetView dashboardFilters={dashboardFilters} effects={effects} panelId={panelId} registry={registry} widget={widget} />,
    }))}
    width={width}
  />
}
...
export function NextPanelClient({ notificationRealtime, payload, registry: registryInput, tenantTransport }: NextPanelClientProps): ReactNode {
  ...
  const notificationTrigger = notificationStore && notificationConfiguration
    ...
    : null
  const dashboardFilters = useMemo(() => createDashboardFilterStore(browserPanelsTransport(), state.panelId, payload.page, payload.bootstrap.locale), [payload.page, state.panelId, payload.bootstrap.locale])
  useEffect(() => () => dashboardFilters?.stop(), [dashboardFilters])
  ...
  return <PanelsLocaleProvider direction={payload.bootstrap.direction} locale={payload.bootstrap.locale}><ReactFeedbackProvider panelId={state.panelId} store={toastStore}><ReactPanelsRenderHookProvider data={payload.page.data} manifest={manifest} registry={registry} scopes={pageScopes}><PanelsPageActionsProvider container={pageActionsContainer}><PanelsPortalProvider container={shell}><div ref={shell} className="hp-panel hp:min-h-svh hp:bg-background hp:text-foreground" data-density={manifest.theme.density} data-holo-panel="" data-navigation={manifest.navigationMode} data-navigation-open={navigationOpen ? 'true' : 'false'} data-panel={state.panelId} data-sidebar-collapsed={sidebarCollapsed ? 'true' : 'false'} data-sidebar-collapsible={manifest.sidebarCollapsible ? 'true' : 'false'} data-sidebar-fully-collapsible={manifest.layout?.sidebarFullyCollapsible ? 'true' : 'false'} data-theme={colorMode} data-width={manifest.layout?.maxContentWidth === 'full' ? 'full' : 'constrained'} dir={payload.bootstrap.direction} lang={payload.bootstrap.locale} style={panelConfigurationVariables(manifest) as CSSProperties}>
    ...
    {renderHook(PanelsRenderHook.FOOTER)}<ReactToastViewport locale={payload.bootstrap.locale} navigate={browserNavigate} panelId={state.panelId} registry={registry} store={toastStore} />{renderHook(PanelsRenderHook.LAYOUT_END)}{renderHook(PanelsRenderHook.BODY_END)}
  </div></PanelsPortalProvider></PanelsPageActionsProvider></ReactPanelsRenderHookProvider></ReactFeedbackProvider></PanelsLocaleProvider>
}
