import { executePanelAuthRequest, panelContentWidthValue, panelThemeVariables, type PanelClientAuthOperation } from '@holo-js/panels-vue'
...
export const PanelMultiFactorPage = defineComponent({
  ...
  setup(props) {
    ...
    return () => h('main', { class: 'hp-auth-page', 'data-holo-panel': '', 'data-theme': props.theme, style: { ...panelThemeVariables({ colors: props.themeColors as Readonly<Record<string, unknown>> | undefined }), ...(props.simplePageMaxContentWidth ? { '--hp-auth-max-width': panelContentWidthValue(props.simplePageMaxContentWidth) } : {}) } }, [h('section', { class: 'hp-auth-card', 'data-slot': 'card' }, [h('div', { 'data-slot': 'card-header' }, [h('span', { class: 'hp-auth-brand-mark' }, [ShadcnIcon('key')]), h('div', [h('p', props.brandName), h('h1', 'Multi-factor authentication'), h('span', `MFA is ${enabled.value ? 'enabled' : 'disabled'}.`)])]), h('div', { 'data-slot': 'card-content' }, [!enabled.value && !manualKey.value ? h(ShadcnButton, { class: 'hp-button hp-button-primary', onClick: () => void begin() }, () => 'Begin enrollment') : null, manualKey.value ? h('section', [h('p', ['Manual key: ', h('code', manualKey.value)]), h('form', { onSubmit: (event: Event) => { event.preventDefault(); void confirm() } }, [verificationFields()[1], h(ShadcnButton, { class: 'hp-button hp-button-primary', type: 'submit' }, () => 'Confirm enrollment')])]) : null, recoveryCodes.value.length ? h('section', { 'aria-label': 'Recovery codes' }, [h('h2', 'Recovery codes'), h('ul', recoveryCodes.value.map(value => h('li', { key: value }, [h('code', value)])))]) : null, enabled.value ? h('form', { onSubmit: (event: Event) => { event.preventDefault(); void protectedAction('mfa-recovery-codes-regenerate') } }, [...verificationFields(), h(ShadcnButton, { class: 'hp-button', type: 'submit' }, () => 'Regenerate recovery codes')]) : null, enabled.value ? h('form', { onSubmit: (event: Event) => { event.preventDefault(); void protectedAction('mfa-disable') } }, [...verificationFields(), h(ShadcnButton, { class: 'hp-button hp-button-destructive', type: 'submit' }, () => 'Disable multi-factor authentication')]) : null, error.value ? h('p', { class: 'hp-auth-error', role: 'alert' }, error.value) : null])])])
  },
})
