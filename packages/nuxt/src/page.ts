...
import { defineAsyncComponent, defineComponent, h, onMounted, onUnmounted, ref, type Component, type PropType, type VNode } from 'vue'
...
function formPage(page: NuxtPanelPageData, panelId: string, registry: ComponentRegistry, schema: ResourceRenderSchema, runtime: PanelPageRuntime): () => VNode {
  ...
  const submit = async (): Promise<void> => {
    await store.submit(async request => {
      ...
      await mutate(runtime, panelId, 'form-submit', mutationPayload({
    ...
    table: {
      actionTransport: {
        async execute(request: { readonly actionId: string, readonly recordId?: number | string }) {
          ...
          await mutate(runtime, panelId, 'action', { actionId: request.actionId, recordIds: [request.recordId], resourceId: schema.resourceId })
        },
      },
...
function viewPage(page: NuxtPanelPageData, panelId: string, registry: ComponentRegistry, schema: ResourceRenderSchema, runtime: PanelPageRuntime): VNode {
  const record = recordFrom(page)
  ...
  const store = new ClientActionStore({
    ...
    transport: {
      async execute() {
        ...
        await mutate(runtime, panelId, 'action', { actionId: action.id, recordIds: [routeValue], resourceId: schema.resourceId })
...
const VueResourcePage = defineComponent({
  ...
  setup(props) {
    ...
    if (props.page.manifest.pageType === 'create' || props.page.manifest.pageType === 'edit') {
      return formPage(props.page, props.panelId, props.registry, schema, runtime)
    }
    return () => props.page.manifest.pageType === 'list'
      ...
      : props.page.manifest.pageType === 'view'
        ? viewPage(props.page, props.panelId, props.registry, schema, runtime)
        : null
  },
})
...
function pageBody(page: NuxtPanelPage, registry: ComponentRegistry, resolveResource: PanelPageProps['resolveResource'], runtime: PanelPageRuntime): VNode {
  ...
  if (component) return h(component, { effects: runtime.effects, key: page.path, page: page.page, panelId: page.bootstrap.manifest.id, registry, transport: runtime.transport })
    ...
    return (): VNode => {
      ...
      return h(PanelsErrorBoundary, {}, {
        default: () => h('div', {
          ...
          class: ['hp-panel', `hp-panel--${bootstrap.manifest.navigationMode}`],
          'data-panels-panel': bootstrap.manifest.id,
          'data-panels-ready': ready.value ? 'true' : 'false',
          'data-panels-theme': bootstrap.manifest.theme.darkMode,
          ...
          h('main', { class: 'hp-panel-content' }, [h('nav', { 'aria-label': 'Breadcrumbs' }, page.breadcrumbs.map((item, index) => h(PanelsLink, { href: item.path, key: `${item.path}:${index}` }, () => item.label))), h('h1', page.heading ?? page.title), page.subheading ? h('p', page.subheading) : null, pageBody(props.page, registry, props.resolveResource, { effects, transport })]),
