...
function relationManagerModel(
  binding: DiscoveredRelationManagerTypeBinding,
...
function renderRelationManagerEntries(
  ...
  return bindings.map((binding) => {
    const model = relationManagerModel(binding, resources, models)
    return [
...
function renderRelationManagerChecks(
  bindings: readonly DiscoveredRelationManagerTypeBinding[],
  resources: ReadonlyMap<string, DiscoveredResourceTypeBinding>,
  models: ReadonlyMap<string, GeneratedModelRegistryEntry>,
): readonly string[] {
  return bindings.map((binding, index) => {
    const model = relationManagerModel(binding, resources, models)
    return `export type RelationManagerRelationship${index} = HoloPanelsRelationName<${exportedType(model.sourcePath, model.exportName ?? 'default')}, ${JSON.stringify(binding.relationship)}>`
  })
}

async function projectModels(projectRoot: string, allowMissing: boolean): Promise<ReadonlyMap<string, GeneratedModelRegistryEntry>> {
  let registryContents: string
  try {
    registryContents = await readFile(resolve(projectRoot, '.holo-js/generated/registry.json'), 'utf8')
  } catch (error) {
    if (isRecord(error) && error.code === 'ENOENT' && allowMissing) {
  ...
  return new Map(registry.models.map(model => [`${model.name}\0${model.tableName}`, model]))
}
...
export async function renderResourceTypeBindings(
  ...
  const models = await projectModels(projectRoot, resourceBindings.length === 0)
...
export async function renderResourceTypeChecks(
  projectRoot: string,
  resourceBindings: readonly DiscoveredResourceTypeBinding[],
  relationManagerBindings: readonly DiscoveredRelationManagerTypeBinding[],
): Promise<GeneratedPanelArtifact> {
  const models = await projectModels(projectRoot, resourceBindings.length === 0)
  const resources = new Map(resourceBindings.map(binding => [resourceKey(binding), binding]))
  const checks = renderRelationManagerChecks(relationManagerBindings, resources, models)
  return {
    path: 'resource-type-checks.ts',
    contents: [
      '// Generated by holo prepare. Do not edit.',
      '',
      "export type HoloPanelsRelationName<TModel extends import('@holo-js/panels-resources').ResourceModelSource, TRelation extends import('@holo-js/panels-resources').ResourceRelationName<TModel>> = TRelation",
      ...checks,
      '',
      'export {}',
      '',
