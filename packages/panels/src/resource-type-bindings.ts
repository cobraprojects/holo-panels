...
function renderRelationManagerChecks(
  ...
  return bindings.map((binding, index) => {
    ...
    const model = models.get(`${resource.modelName}\0${resource.tableName}`)
    if (!model) throw new Error(`[Holo Panels] Parent resource ${resource.projectPath} has no generated Holo model.`)
    return `type RelationManagerRelationship${index} = HoloPanelsRelationName<${exportedType(model.sourcePath, model.exportName ?? 'default')}, ${JSON.stringify(binding.relationship)}>`
  })
}
...
export async function renderResourceTypeChecks(
  ...
  return {
    ...
    contents: [
      '// Generated by holo prepare. Do not edit.',
      '',
      "type HoloPanelsRelationName<TModel extends import('@holo-js/panels-resources').ResourceModelSource, TRelation extends import('@holo-js/panels-resources').ResourceRelationName<TModel>> = TRelation",
      ...checks,
      '',
      'export {}',
      '',
