...
export function ReactNavigationSearchRenderer(props: ReactNavigationSearchRendererProps): ReactNode {
  ...
  return <div className={`hp-navigation-search hp-navigation-search--${navigation.manifest.layout}`} data-panels-component="navigation-search">
    <Button aria-expanded={navigation.menuOpen} aria-label={translate('navigation.toggle')} onClick={() => props.navigation.toggleMenu()} type="button">{translate('navigation.menu')}</Button>
    {navigation.manifest.panels.length > 1 ? <label>{translate('navigation.panel')}<NativeSelect aria-label={translate('navigation.panel')} onChange={event => props.onNavigate?.(props.navigation.switchPanel(event.currentTarget.value))} value={navigation.manifest.panelId}>{navigation.manifest.panels.map(panel => <option key={panel.id} value={panel.id}>{panel.label}</option>)}</NativeSelect></label> : null}
    <nav aria-label={translate('navigation.label')} hidden={!navigation.menuOpen && navigation.manifest.layout === 'sidebar'} onKeyDown={keyNavigation}>
    ...
    <div className="hp-global-search" role="search">
      <label>{translate('search.label')}<Input aria-controls="hp-global-search-results" aria-expanded={search.open} onChange={event => props.search.input(event.currentTarget.value)} onFocus={() => props.search.open()} onKeyDown={keySearch} placeholder={translate('search.placeholder')} role="combobox" value={search.term} /></label>
      <kbd>⌘/Ctrl K</kbd>
      {search.loading ? <span aria-live="polite" role="status">{translate('search.loading')}</span> : null}
