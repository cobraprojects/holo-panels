...
export function ReactDashboardFilters({ store, panelId, registry }: { readonly store: DashboardFilterStore, readonly panelId: string, readonly registry: ComponentRegistry }): ReactNode {
  ...
  return <form aria-label={translate('widgets.dashboardFilters')} className="hp-dashboard-filters hp:grid hp:gap-4" noValidate onSubmit={event => { event.preventDefault(); void store.submit() }}>
    ...
    <div className="hp:flex hp:gap-2"><Button disabled={state.submitting} type="submit">{translate('tables.applyFilters')}</Button><Button disabled={state.submitting} onClick={() => void store.submit(true)} type="button" variant="outline">{translate('tables.resetFilters')}</Button></div>
