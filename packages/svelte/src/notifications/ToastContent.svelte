...
<Card class="hp-notification-toast hp:relative hp:w-full hp:border-0 hp:shadow-none" data-color={toast.color ?? undefined} data-persistent={toast.persistent || undefined} data-status={toast.status}>
  <CardHeader class="hp:gap-1 hp:pr-10">
    <CardTitle class="hp:flex hp:items-center hp:gap-2 hp:text-sm">{#if toast.icon}<Icon name={toast.icon} />{/if}{toast.title}</CardTitle>
