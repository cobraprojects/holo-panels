<script lang="ts" generics="TRecord extends object">
  ...
  import type { SvelteCustomColumnProps, SvelteTableColumn } from './types'
  ...
  const rendered = $derived(column.render?.(value, record))
  const tooltip = $derived(formatters.find(formatter => formatter.kind === 'tooltip')?.value)
  const url = $derived(safeTableUrl(formatters.find(formatter => formatter.kind === 'url')?.value))
...
{#snippet content()}
  ...
  {:else if custom}
    ...
    {:else if CustomRenderer}
      <CustomRenderer {...customProperties} {column} {record} {value} />
    {/if}
