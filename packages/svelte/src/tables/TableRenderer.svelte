<script lang="ts" generics="TRecord extends object, TRecordId extends TableRecordId">
  ...
  const orderedFilters = $derived.by(() => {
    ...
    return filterPresentation.schema.components.flatMap(component => {
      const filter = component.statePath ? byId.get(component.statePath) : undefined
      return filter ? [filter] : []
    })
  })
  const hasSelection = $derived($snapshotStore.selection.mode === 'all-matching' || $snapshotStore.selection.selectedRecordIds.length > 0)
...
<section aria-labelledby={captionId} class="hp-table-view" data-panels-component="table">
  ...
  <div class="hp-table-toolbar">
    ...
    <div class="hp-column-manager">
      <button type="button" aria-expanded={columnsOpen} aria-haspopup="menu" onclick={() => { columnsOpen = !columnsOpen }}>Columns</button>
      {#if columnsOpen}
    ...
    {#each table.transfers ?? [] as manifest (manifest.id)}
      <TransferAction {manifest} table={table as SvelteTableRendererProps<object, TableRecordId>} />
    {/each}
  </div>
  ...
  {#if (table.filters?.length ?? 0) > 0}
    {#if filterPlacement === 'inline'}
      {@render filterForm()}
    {:else}
      <button type="button" aria-expanded={filtersOpen} aria-haspopup="dialog" onclick={() => { filtersOpen = !filtersOpen }}>Filters</button>
  ...
  {#if $snapshotStore.records.length > 0}
    <div class="hp-table-responsive" {...regionAttributes}>
      <table>
        <caption class="hp-visually-hidden">{table.caption}</caption>
        <thead>
          <tr>
            <th scope="col"><input aria-label="Select page" type="checkbox" checked={selectedOnPage} onchange={(event) => table.store.selectPage(recordIds, (event.currentTarget as HTMLInputElement).checked)} /></th>
            ...
            {#each table.groups as group (group.key)}
              ...
              <tr class="hp-table-group">
                <th colspan={columns.length + 2} scope="rowgroup">
                  {#if group.collapsible}<button type="button" aria-expanded={!collapsed} onclick={() => toggleGroup(group.key)}>{group.title}</button>{:else}{group.title}{/if}
              ...
              {#if !collapsed}
                {#each group.records as record (table.getRecordId(record))}
                  ...
                  <tr>
                    <td data-label="Select"><input type="checkbox" aria-label="Select record {String(recordId)}" checked={table.store.isSelected(recordId)} onchange={(event) => table.store.selectRecord(recordId, (event.currentTarget as HTMLInputElement).checked)} /></td>
                    ...
                    <td data-label="Actions">{#each rowActions as action (action.id)}<ActionButton {action} {record} {table} />{/each}</td>
                  </tr>
                {/each}
              {/if}
              {#each group.summaries ?? [] as summary (summary.id)}<tr><th colspan={columns.length + 2} scope="row">{summary.label}: {summary.value}</th></tr>{/each}
            {/each}
          {:else}
        ...
        {#if (table.summaries?.length ?? 0) > 0}
          <tfoot>{#each table.summaries ?? [] as summary (summary.id)}<tr><th colspan={Math.max(1, columns.length + 2)} scope="row">{summary.label}: {summary.value}</th></tr>{/each}</tfoot>
        {/if}
      </table>
    </div>
  {/if}
  ...
  <nav aria-label="Table pagination" class="hp-table-pagination">
    <button type="button" aria-label="Previous page" disabled={$snapshotStore.page <= 1} onclick={() => changePage($snapshotStore.page - 1)}>Previous</button>
    <span>Page {$snapshotStore.page} of {pages}</span>
    <button type="button" aria-label="Next page" disabled={$snapshotStore.page >= pages} onclick={() => changePage($snapshotStore.page + 1)}>Next</button>
  </nav>
</section>
