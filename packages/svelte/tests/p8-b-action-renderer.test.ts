...
describe('P8-B Svelte action renderer', () => {
  it('renders grouped triggers and complete slide-over presentation with nested actions and slots', () => {
    ...
    flushClient()
    Array.from(container.querySelectorAll('button')).find(button => button.textContent === 'Restore')?.click()
    flushClient()
    ...
    expect(dialog?.textContent).toContain('Restore post')
    expect(dialog?.textContent).toContain('Review restoration')
