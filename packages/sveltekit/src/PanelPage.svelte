...
>
  ...
  {#if TopbarComponent}<TopbarComponent actor={data.panel.actor} manifest={data.panel.manifest} page={data.page} />{:else}<header class="hp-panel-header hp:sticky hp:top-0 hp:z-20 hp:col-start-2 hp:row-start-1 hp:flex hp:h-16 hp:min-w-0 hp:items-center hp:gap-2 hp:border-b hp:bg-background hp:px-4">
    ...
    {#if data.panel.manifest.navigationEnabled !== false}{#if data.panel.manifest.navigationMode === 'sidebar' && !SidebarComponent}<SidebarTrigger aria-label={translate('navigation.toggle')} class="hp-panel-navigation-toggle hp-panel-topbar-start-action" id={navigationToggleId} label={translate('sidebar.toggle')} />{:else}<Button aria-controls={navigationId} aria-expanded={navigationOpen} aria-label={translate('navigation.toggle')} class="hp-panel-navigation-toggle hp-panel-topbar-start-action" id={navigationToggleId} onclick={toggleNavigation} size="icon" type="button" variant="ghost"><Icon aria-hidden="true" name="menu" /></Button>{/if}{/if}
    ...
    {#if data.panel.manifest.navigationEnabled !== false && data.panel.manifest.navigationMode === 'topbar'}
      <nav aria-label={translate('navigation.label')} class="hp-panel-navigation hp-panel-navigation--topbar hp-panel-navigation-body hp-panel-topbar-center" data-open={navigationOpen} id={navigationId}>
    ...
    <div class="hp-panel-header-actions hp-panel-topbar-end hp-panel-actions--compact">
      ...
      {/if}
      {#if data.panel.manifest.navigationMode === 'topbar' && data.panel.tenancy && data.panel.manifest.tenancy?.switcher !== false}<div class="hp-panel-tenant-action hp-panel-action--compact"><SvelteTenantSwitcher shell={{ locale: data.panel.locale, onSwitched: () => window.location.reload(), store: tenantStore, transport: tenantTransport }} /></div>{/if}
  ...
  {#if SidebarComponent}<SidebarComponent actor={data.panel.actor} manifest={data.panel.manifest} page={data.page} />{:else}<Sidebar class="hp-panel-sidebar" closeLabel={translate('actions.close')} collapsible={data.panel.manifest.sidebarCollapsible ? (data.panel.manifest.layout?.sidebarFullyCollapsible ? 'offcanvas' : 'icon') : 'none'} mobileDescription={translate('sidebar.description')} mobileTitle={translate('sidebar.title')} side={data.panel.direction === 'rtl' ? 'right' : 'left'}>
    ...
    <SidebarContent><nav aria-label={translate('navigation.label')} class="hp-panel-navigation hp-panel-navigation-body" id={navigationId}>
