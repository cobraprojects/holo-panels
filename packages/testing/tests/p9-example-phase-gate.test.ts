...
describe('P9 example phase gate', () => {
  ...
  it('executes Next create, list, view, edit, delete, and tenant isolation through the exported example runtime and page resolver', async () => {
    ...
    await runtime.execute?.({ operation: 'action', panelId: 'admin', payload: { actionId: 'delete', input: {}, mount: 'record', recordIds: [4], resourceId: 'posts', source: 'edit' }, request: scope.request, scope })
  ...
  it('executes Nuxt list filters, view, CRUD, policy denial, and safe validation through the exported runtime', async () => {
    ...
    await runtime.execute(context('action', admin, { actionId: 'delete', input: {}, mount: 'record', recordIds: [4], resourceId: 'posts', source: 'edit' }))
  ...
  it('executes SvelteKit list filters, view, CRUD, dependencies, policy denial, and safe validation through the exported registry', async () => {
    ...
    await registry.operations?.action?.({ event: svelteEvent('/holo/panels/admin/action'), holo, operation: 'action', panelId: 'admin', payload: { actionId: 'delete', input: {}, mount: 'record', recordIds: [4], resourceId: 'posts', source: 'edit' }, scope: svelteScope(svelteAdmin), tenant: 'tenant-a' })
