...
const RelationActions = defineComponent({
  ...
  setup(props) {
    ...
    const host = computed(() => createRelationActionHost({
      ...
      locale: locale(), loadOptions: props.relations.loadOptions, manager: props.manager, panelId: props.relations.panelId, record: props.record, selectedIds: props.selectedIds,
    }))
...
const RelationPanel = defineComponent({
  ...
  setup(props) {
    ...
    const pageActions = shallowRef<NonNullable<ClientRelationManager['recordActions']>>(manager.records.map(record => ({ actions: relationActionManifests(manager, record, false, locale()), recordId: record.id })))
    ...
    const refresh = (): void => {
      ...
      void props.relations.onTableQuery({ managerId: manager.id, query, selection: store.selectionPayload() }).then((page) => {
        ...
        pageActions.value = page.records.map(record => ({ actions: relationActionManifests(pageManager, record, false, locale()), recordId: record.id }))
    ...
    return () => {
      return h(Card, { 'aria-label': manager.label, class: 'hp-relation-manager', 'data-empty': manager.records.length === 0 || undefined, 'data-relation-manager': manager.id, role: 'region' }, () => [
        h(CardHeader, { class: 'hp-relation-manager-header', 'data-slot': 'relation-manager-header' }, () => [
          ...
          props.relations.onOperation ? h('div', { 'aria-label': translate('relations.actions', { label: manager.label }), class: 'hp-relation-toolbar', 'data-slot': 'relation-toolbar', role: 'group' }, h(RelationActions, { manager, relations: props.relations })) : null,
        ]),
        h(CardContent, {}, () => h(VueTableRenderer, { table: {
          ...
          actions: actions.value, caption: manager.label, columns, emptyMessage: manager.emptyMessage ?? translate('relations.empty', { label: manager.label.toLocaleLowerCase() }), filters, getRecordId: (record: ClientRelationRecord) => record.id, onQueryChange: props.relations.onTableQuery ? refresh : undefined, panelId: props.relations.panelId, registry: props.relations.registry, store,
        } })),
      ])
    }
  },
  ...
  setup(componentProps) {
    ...
    return () => {
      ...
      return h('div', { class: 'hp-relations', 'data-panels-component': 'relation-managers' }, [
        ...
        ...layout.tabGroups.map(group => h(Tabs, { 'aria-label': group.label ?? translate('relations.records'), class: 'hp-relation-tabs', key: group.id, modelValue: group.activeId, 'onUpdate:modelValue': (value: string | number) => select(group.id, String(value)) }, () => [
        ...
        layout.pages.length > 0 ? h('nav', { 'aria-label': translate('relations.pages'), class: 'hp-relation-pages' }, layout.pages.map(manager => h('a', { href: manager.url!, key: manager.id }, `${manager.label}${manager.badge !== null ? ` (${manager.badge})` : ''}`))) : null,
      ])
    }
  },
})
