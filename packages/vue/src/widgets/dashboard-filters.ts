...
export const VueDashboardFilters = defineComponent({
  ...
  setup(props) {
    ...
    return () => h('form', { 'aria-label': 'Dashboard filters', class: 'hp-dashboard-filters hp:grid hp:gap-4', novalidate: true, onSubmit: (event: Event) => { event.preventDefault(); void props.store.submit() } }, [
      ...
      h('div', { class: 'hp:flex hp:gap-2' }, [h(Button, { disabled: state.value.submitting, type: 'submit' }, () => 'Apply filters'), h(Button, { disabled: state.value.submitting, onClick: () => void props.store.submit(true), type: 'button', variant: 'outline' }, () => 'Reset filters')]),
