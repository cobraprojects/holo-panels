...
describe('P13 Vue notification inbox trigger', () => {
  ...
  it('keeps the inbox mounted while closed and supports accessible topbar interaction', async () => {
    ...
    mounted.push({ app, container })
    app.mount(container)
    await vi.waitFor(() => expect(container.querySelector('.hp-notification-inbox-trigger-button')?.getAttribute('aria-label')).toBe('Notifications, 1 unread'))
  ...
  it('maps sidebar placement and restores trigger focus after Escape and outside clicks', async () => {
    ...
    mounted.push({ app, container })
    app.mount(container)
    await vi.waitFor(() => expect(document.querySelector<HTMLElement>('.hp-notification-inbox')?.dataset.placement).toBe('sidebar'))
    const button = container.querySelector<HTMLButtonElement>('.hp-notification-inbox-trigger-button')
    const content = document.querySelector<HTMLDivElement>('.hp-notification-inbox-trigger-content')
