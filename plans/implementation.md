...
- [x] **P13 phase gate:** a queued database notification appears through polling and realtime, can be acted on/read/deleted, and remains isolated between admin and vendor guards.
...
- [ ] Publish prerelease packages and test installation from the registry. npm authentication succeeds and Holo-JS 0.3.10 is published. The `test:registry-release` gate is ready to bootstrap the published Holo CLI, create clean Next, Nuxt, and SvelteKit applications without local dependency references, install the exact Holo Panels version, and exercise plugin activation, preparation, panel/resource generation, adapter selection, Shield, idempotency, and safe uninstall. Its fail-closed registry/version policy passes 4/4 tests, and the refactored packed path passes all three frameworks. Holo Panels 0.1.0-next.0 publication and observation of the registry gate remain pending.
- [x] Publish migration/upgrade guide and changelog. Evidence: `docs/testing-deployment-upgrades.md` documents clean installation, generated artifacts, deployment, lockstep releases, safe upgrades, migrations, and uninstall; `CHANGELOG.md` records the current unreleased workspace and remaining prerelease work.
- [ ] Release `@holo-js/panels` and its workspace family. The source-resolved dry run successfully simulates all 14 public packages with the `next` tag, verifies the canonical MIT license in every tarball, and passes all clean packed consumers; real publication remains pending the final protected release gate.
...
- [ ] All code quality, security, package, framework, and acceptance gates pass.
