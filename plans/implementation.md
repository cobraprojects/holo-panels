...
- [x] #26 release gate: prove the dashboard refactor from a clean checkout through public package contracts, clean generated applications, and shared built-application behavior.

Ticket #26 closure audit on 2026-09-05: the first full browser run exposed duplicate accessible upload-error announcements in all three renderers. Commit `647d099` preserves the shared form error state while preventing React, Vue, and Svelte upload controls from announcing the same store error twice. The renderer regressions passed through Vitest's JSON reporter with 14 React, 11 Vue, and 5 Svelte tests. TypeScript language-server diagnostics reported no errors for the four changed React and Vue executable files, and Svelte diagnostics reported zero errors and warnings for its changed source and test. A fresh detached checkout with a frozen lockfile and the documented adjacent Holo-JS link passed strict typechecks and builds for all 21 packages, ESLint, and all 1,361 tests across 446 workspace suites. The 33 tooling tests, architecture and dependency policies, conditional exports, the 161-topic parity matrix, example parity, publish metadata, all three packed generation lifecycles, every packed-package import, independent framework and optional-peer consumer typechecks, public plugin examples, and the third-party plugin smoke test passed. The isolated-linker browser bootstrap exposed an undeclared Svelte DOM type dependency; commit `affa816` makes that declaration-build environment explicit. From the clean checkout, all 126 shared Playwright checks then passed across built Next.js, Nuxt, and SvelteKit applications. The browser suite covers English/LTR and Arabic/RTL, desktop and mobile widths, light and dark geometry, framework-native navigation, dashboard and resource behavior, and sanitized failures without stack paths, disposed-session messages, raw transport errors, or inline internal action failures.
...
- [x] Implement table widgets by composing the table engine. Ticket #23 binds resources to shared queries, controls, actions, selection, pagination, and framework renderers. Production-browser acceptance passes in Next.js, Nuxt, and SvelteKit.
...
- [x] **P12 phase gate:** stats, chart, table, and custom widgets render on dashboard and resource pages across all frameworks. Revalidated for ticket #23 on 2026-08-31 through the complete workspace and packed-package gates and all 126 production-browser journeys, including nine new Table/Custom widget cases.
...
Ticket #23 completion on 2026-08-31 adds the approved `.table(PostResource)` API, shared table state and renderers, page/widget authorization for table requests, registered Custom widget renderers, generators, watched renderer registrations, multiple dashboards, and page-specific resource widgets. Commits `dd2edba`, `1fa152d`, `c19a214`, and `b30f724` are local and have not been pushed. Table actions and Custom widget state survive polling. A changed bound dashboard selection scope clears all-matching selection and rejects stale bulk actions instead of silently changing their target records.

Final ticket #23 validation passed 1,351 tests across 454 workspace suites, all 126 production-browser journeys, all 21 package builds and typechecks, all three example typechecks and production builds, browser-test typechecking, zero-error language-service diagnostics on 69 changed TypeScript files, zero-warning Svelte diagnostics, ESLint, architecture/dependency/public API checks, conditional exports, and example parity. All 21 packed packages installed and imported successfully; independent consumers and Next/Nuxt/SvelteKit fixtures typechecked, and all three packed installation lifecycles passed. The connected client-controller/SQLite-server regression covers bound queries, tenant and record authorization, valid and forged bulk selections, stale-action rejection, and selection invalidation after a dashboard scope change. Runtime tests cover lifecycle restart, selection retention, grouping removal, and deferred filter drafts. Final Standards and Spec reviews each have zero remaining findings. The previously recorded unrelated Vue Sonner SSR log error remains; all browser journeys pass.
