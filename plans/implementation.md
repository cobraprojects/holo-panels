...
The ticket remains open. Dynamic table callbacks still use static fallbacks, form action getters are unused, navigation actions bypass parts of the shared lifecycle, and relation controls ignore configured confirmation overrides. Database notification actions now resolve through discovered resource owners without a separate registration call. Executable toast actions, request-only action closures, and notification children declared through `registerModalActions(...)` are not implemented. The earlier isolated renderer tests do not prove these remaining generated application paths.

The presentation increment at `8c3aa0b` passed language-service diagnostics, changed-file ESLint, all package builds and typechecks, all 21 package JSON Vitest suites, architecture/dependency/publish checks, and example parity. Focused regressions cover absent and partial input, complete execution input, literal and callback visibility, actor-specific list/create page presentation, same-ID mount isolation, and preserved static presentation fields.

The entry/widget/list-host increment passed language-service diagnostics for changed TypeScript files, Svelte diagnostics, changed-file ESLint, all package builds and typechecks, and preparation for all three example apps. The workspace JSON Vitest run completed after refreshing the declaration fixture and generated API index and restoring existing Svelte stat-action button rendering. Architecture, dependency, publish-metadata, and example-parity checks passed. Focused tests cover nested entry registration, shared confirmation, widget transport, cross-request retries and rate limits, permission denial and shared references, list actions without record IDs, and partial/network failure feedback. ESLint does not configure `.svelte` files; `svelte-check` reported zero errors and warnings. Standards and spec re-reviews found no remaining defects in this increment. The remaining host-integration gaps above still block ticket #12 and the P8 gate.

The database-notification increment keeps `Notification.make().actions([retryAction])` unchanged. Resource-owned static actions acquire durable owner references automatically. Active panel operations send notifications through Holo without application registration. Inbox actions resolve current discovered definitions and use the shared engine, client store, and React/Vue/Svelte action renderers. Tests reload resource definitions from a saved payload, reject foreign or unattached notification actions, reauthorize retries, deduplicate actors with custom primary keys, and exercise confirmation before execution in all three renderers. Executable toasts, request-only closures, and nested notification modal actions remain open.

This increment passed language-service diagnostics for all 43 changed TypeScript files, Svelte diagnostics, all 21 package builds and typechecks, changed-file ESLint, full workspace ESLint with an 8 GiB heap, and the full workspace JSON Vitest run. Architecture, dependency, publish-metadata, conditional-export, generated API reference, and example-parity checks passed. All three example apps prepared and typechecked successfully. Standards re-review found no remaining findings. Spec re-review found no blockers within this increment and confirmed that nested notification modal actions remain incomplete. These checks used the adjacent Holo-JS prerequisite above, not a published release containing it.
...
- [x] Implement stats widgets with value, description, trend, chart sparkline, icon, color, and URL/action.
...
- [x] Implement status, title, body, icon, color, duration, persistent, close, and actions.
...
- [x] **P13 phase gate:** a queued database notification appears through polling and realtime, can be acted on/read/deleted, and remains isolated between admin and vendor guards. Revalidated on 2026-08-27: the action engine emits one sanitized Panel notification effect with safe defaults or the action's configured presentation, while React, Vue, and Svelte render no inline transport feedback; notification, inbox, polling/realtime, keyboard, Holo SQLite, and cross-framework suites pass; and the full 21-package typecheck, lint, tests, architecture, dependency, publication, parity, and example checks pass.
