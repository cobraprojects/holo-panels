...
- [x] Ticket #19: the `make:resource --simple` option emits one `ManageRecords` page; generated Next.js, Nuxt, and SvelteKit resources typecheck with the same table and modal CRUD contract. Manage pages load the record table as page content, create and edit reuse the resource form and shared action/notification lifecycle, view projects the configured infolist as read-only modal state, destructive confirmation defaults remain unchanged, and successful modal mutations refresh the table without navigation. Validated on 2026-08-29 with 1,279 workspace tests across 442 suites, all 21 package builds and typechecks, zero-warning Svelte diagnostics, ESLint, architecture, dependency, publish, and example-parity checks, plus focused generator, generated-page, and React/Vue/Svelte modal suites.
