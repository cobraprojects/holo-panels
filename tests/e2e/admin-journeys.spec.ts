...
test.describe('authenticated admin journeys', () => {
  ...
  test('runs authentication, Resource CRUD, and widgets in Arabic and RTL', async ({ browser }, testInfo) => {
    ...
    try {
      ...
      for (const width of [1280, 390]) {
        ...
        await expect(page.getByPlaceholder('البحث في السجلات…')).toBeVisible()
        const suffix = `${testInfo.project.name}-${width}-${Date.now()}`
        const name = `فئة ${suffix}`
        await page.getByRole('button', { name: 'إنشاء', exact: true }).click()
        const create = page.getByRole('dialog')
        await create.getByRole('button', { name: 'إنشاء', exact: true }).click()
        await expect(create.locator('[data-field-path="name"] [role="alert"]')).toContainText('هذا الحقل مطلوب.')
        await create.getByRole('textbox', { name: 'Name', exact: true }).fill(name)
        await create.getByRole('textbox', { name: 'Slug', exact: true }).fill(`arabic-${suffix}`)
        await create.getByRole('button', { name: 'إنشاء', exact: true }).click()
        await expect(create).toHaveCount(0)
        const created = page.getByRole('row').filter({ hasText: name })
        await expect(created).toBeVisible()
        await created.getByRole('button', { name: 'إجراءات الصف', exact: true }).click()
        await page.getByRole('menuitem', { name: 'تعديل', exact: true }).click()
        const edit = page.getByRole('dialog')
        await edit.getByRole('textbox', { name: 'Name', exact: true }).fill(`${name} معدلة`)
        await edit.getByRole('button', { name: 'حفظ', exact: true }).click()
        await expect(edit).toHaveCount(0)
        await created.getByRole('button', { name: 'إجراءات الصف', exact: true }).click()
        await page.getByRole('menuitem', { name: 'حذف', exact: true }).click()
        const confirmation = page.getByRole('alertdialog')
        await expect(confirmation).toContainText('هل أنت متأكد من حذف هذا السجل؟')
        await confirmation.getByRole('button', { name: 'تأكيد', exact: true }).click()
        await expect(created).toHaveCount(0)

        await gotoPanelPage(page, '/admin/metrics')
        const chart = page.getByRole('table', { name: 'Publishing totals' })
        await expect(chart).toBeVisible()
        await expect(chart.getByRole('columnheader', { name: 'التسمية' })).toBeVisible()
        expect(await chart.evaluate(element => getComputedStyle(element).direction)).toBe('rtl')
        ...
        await page.getByRole('button', { name: 'إعادة تعيين عوامل التصفية' }).click()
        await expect(page.getByLabel('Search posts')).toHaveValue('')

        await gotoPanelPage(page, '/admin/widgets')
        const tableWidget = page.locator('[data-widget-id="recent-posts"], [data-panels-widget="recent-posts"]')
        const customWidget = page.locator('[data-widget-id="content-notice"], [data-panels-widget="content-notice"]')
        const widgetFilters = page.getByRole('form', { name: 'عوامل تصفية لوحة المعلومات', exact: true })
        await expect(tableWidget.getByPlaceholder('البحث في السجلات…')).toBeVisible()
        await page.getByLabel('Search posts', { exact: true }).fill('draft')
        await widgetFilters.getByRole('button', { name: 'تطبيق عوامل التصفية' }).click()
        await expect(tableWidget.getByRole('row').filter({ hasText: 'Acme draft' })).toBeVisible()
        await expect(customWidget).toContainText('Publishing search: draft')
        await widgetFilters.getByRole('button', { name: 'إعادة تعيين عوامل التصفية' }).click()
      }

      await page.setViewportSize({ width: 1280, height: 900 })
      const relationTitle = `Arabic relations ${testInfo.project.name}-${Date.now()}`
      await gotoPanelPage(page, '/admin/posts/create')
      await page.getByRole('textbox', { name: /^Title/iu }).fill(relationTitle)
      await selectField(page, 'Category', ['Guides'])
      await selectField(page, 'City', ['Cairo'])
      const saved = page.waitForResponse(response => response.url().endsWith('/form-submit'))
      await page.locator('.hp-resource-form').getByRole('button', { name: 'إنشاء', exact: true }).click()
      expect((await saved).ok()).toBe(true)
      await gotoPanelPage(page, '/admin/posts')
      const relationOwner = page.getByRole('row').filter({ hasText: relationTitle })
      await relationOwner.getByRole('button', { name: 'إجراءات الصف', exact: true }).click()
      await page.getByRole('menuitem', { name: 'تعديل', exact: true }).click()
      const tags = page.locator('[data-relation-manager="tags"]')
      await tags.getByRole('button', { name: 'إرفاق', exact: true }).click()
      const relatedTag = page.getByRole('dialog').getByRole('combobox').first()
      await expect.poll(() => relatedTag.locator('option').count()).toBeGreaterThan(1)
      await relatedTag.selectOption({ label: 'TypeScript' })
      await page.getByRole('spinbutton', { name: 'Position', exact: true }).fill('2')
      await page.getByRole('dialog').getByRole('button', { name: 'إرفاق', exact: true }).click()
      const attachedTag = tags.getByRole('row').filter({ hasText: 'TypeScript' })
      await expect(attachedTag).toBeVisible()
      await attachedTag.getByRole('button', { name: 'إجراءات الصف', exact: true }).click()
      await page.getByRole('menuitem', { name: 'فصل', exact: true }).click()
      await page.getByRole('alertdialog').getByRole('button', { name: 'تأكيد', exact: true }).click()
      await expect(attachedTag).toHaveCount(0)
      await gotoPanelPage(page, '/admin/posts')
      await relationOwner.getByRole('button', { name: 'إجراءات الصف', exact: true }).click()
      await page.getByRole('menuitem', { name: 'حذف', exact: true }).click()
      await page.getByRole('alertdialog').getByRole('button', { name: 'تأكيد', exact: true }).click()
      await expect(relationOwner).toHaveCount(0)
    } finally {
  ...
  test('composes Table and Custom widgets with dashboard filters, polling and session restoration', async ({ page }) => {
    await login(page)
    await gotoPanelPage(page, '/admin/widgets')
    const tableWidget = page.locator('[data-widget-id="recent-posts"], [data-panels-widget="recent-posts"]')
    const customWidget = page.locator('[data-widget-id="content-notice"], [data-panels-widget="content-notice"]')
    await tableWidget.scrollIntoViewIfNeeded()
    await expect(tableWidget.getByRole('table', { name: 'Posts', exact: true })).toBeVisible()
    await expect(customWidget).toContainText('All publishing records')
    await customWidget.getByText('Publishing details', { exact: true }).click()
    await expect(customWidget.locator('details')).toHaveAttribute('open', '')
    await page.getByLabel('Search posts', { exact: true }).fill('draft')
    await page.getByRole('form', { name: 'Dashboard filters', exact: true }).getByRole('button', { name: 'Apply filters', exact: true }).click()
    await expect(tableWidget.getByRole('row').filter({ hasText: 'Acme draft' })).toBeVisible()
    await expect(tableWidget.getByRole('row').filter({ hasText: 'Building with Holo Panels' })).toHaveCount(0)
    await expect(customWidget).toContainText('Publishing search: draft')
    const selected = tableWidget.getByRole('checkbox', { name: /^Select record /u }).first()
    await selected.click()
    await page.waitForResponse(response => response.url().endsWith('/holo/panels/admin/page-data') && response.request().postData()?.includes('recent-posts') === true)
    await expect(selected).toBeChecked()
    await expect(customWidget.locator('details')).toHaveAttribute('open', '')
    await page.reload()
    await expect(page.getByLabel('Search posts', { exact: true })).toHaveValue('draft')
    await expect(customWidget).toContainText('Publishing search: draft')
    await expect(tableWidget.getByRole('row').filter({ hasText: 'Acme draft' })).toBeVisible()
    await page.getByRole('form', { name: 'Dashboard filters', exact: true }).getByRole('button', { name: 'Reset filters', exact: true }).click()
    await expect(customWidget).toContainText('All publishing records')
    await expect(tableWidget.getByRole('row').filter({ hasText: 'Building with Holo Panels' })).toBeVisible()
  })

  test('uses shared Table widget search and actions on another dashboard', async ({ page }) => {
    await login(page)
    await gotoPanelPage(page, '/admin/reports')
    const widget = page.locator('[data-widget-id="recent-posts"], [data-panels-widget="recent-posts"]')
    await widget.scrollIntoViewIfNeeded()
    await expect(widget.getByRole('table', { name: 'Posts', exact: true })).toBeVisible()
    await widget.getByRole('searchbox', { name: 'Search', exact: true }).fill('draft')
    await expect(widget.getByRole('row').filter({ hasText: 'Acme draft' })).toBeVisible()
    await expect(widget.getByRole('row').filter({ hasText: 'Building with Holo Panels' })).toHaveCount(0)
    await widget.getByRole('checkbox', { name: /^Select record /u }).first().click()
    await widget.getByRole('button', { name: 'Bulk actions', exact: true }).click()
    await page.getByRole('menuitem', { name: 'Publish selected', exact: true }).click()
    await page.waitForResponse(response => response.url().endsWith('/holo/panels/admin/page-data') && response.request().postData()?.includes('recent-posts') === true)
    await expect(page.getByRole('button', { name: 'Confirm', exact: true })).toBeVisible()
    const responsePromise = page.waitForResponse(response => response.url().endsWith('/holo/panels/admin/action') && response.request().method() === 'POST')
    await page.getByRole('button', { name: 'Confirm', exact: true }).click()
    expect(await (await responsePromise).json()).toMatchObject({ ok: true, data: { status: 'succeeded' } })
    await expect(widget.getByRole('button', { name: 'Next page', exact: true })).toBeDisabled()
    await widget.getByRole('row').filter({ hasText: 'Acme draft' }).getByRole('button', { name: 'Row actions', exact: true }).click()
    await page.getByRole('menuitem', { name: 'View', exact: true }).click()
    await expect(page).toHaveURL(/\/admin\/posts\/post-acme-draft$/u)
  })

  test('renders Table and Custom widgets with authorized resource context', async ({ page }) => {
    await login(page)
    await gotoPanelPage(page, '/admin/posts?search=draft')
    await expect(page.getByRole('region', { name: 'Page header widgets' })).toContainText('Search: draft')
    const widgets = page.getByRole('region', { name: 'Page footer widgets' })
    await expect(widgets).toHaveCount(0)
    await gotoPanelPage(page, '/admin/posts/post-acme-draft')
    await widgets.scrollIntoViewIfNeeded()
    await expect(widgets).toContainText('Publishing record: Acme draft')
    await expect(widgets.getByRole('table', { name: 'Posts', exact: true })).toBeVisible()
    const denied = await panelOperation(page, 'page-data', { pageId: 'posts-view', widgetId: 'record-context', parameters: { record: 'post-globex-platform' } })
    expect(denied.body).toMatchObject({ ok: false })
    expect(JSON.stringify(denied.body)).not.toContain('Globex platform')
  })
  ...
  test('keeps reactive slugs and the upload lifecycle working on create and edit', async ({ page }, testInfo) => {
    test.setTimeout(60_000)
    const errors = collectDisposedSessionErrors(page)
    await login(page)
    const suffix = `${testInfo.project.name}-${Date.now()}`
    const title = `Upload lifecycle ${suffix}`
    const image = { buffer: Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Y9Z8ZkAAAAASUVORK5CYII=', 'base64'), mimeType: 'image/png', name: `${suffix}.png` }
    await gotoPanelPage(page, '/admin/posts/create')
    const titleInput = page.getByRole('textbox', { name: /^Title/iu })
    const slugInput = page.getByRole('textbox', { name: /^Slug/iu })
    const upload = page.locator('[data-field-path="featuredMediaId"]')
    const fileInput = page.locator('input[type="file"]')
    await slugInput.focus()
    await titleInput.fill(title)
    await expect(slugInput).toHaveValue(`upload-lifecycle-${suffix}`)
    await expect(titleInput).toBeFocused()
    await slugInput.fill(`editorial-${suffix}`)
    await titleInput.fill(`${title} changed`)
    await expect(slugInput).toHaveValue(`editorial-${suffix}`)
    await selectField(page, 'Category', ['Guides'])
    await selectField(page, 'City', ['Cairo'])

    await fileInput.setInputFiles({ ...image, name: 'unsafe.exe' })
    await expect(upload.getByRole('alert')).toContainText('extension')
    await fileInput.setInputFiles({ ...image, buffer: Buffer.from('not an image'), name: 'spoofed.png' })
    await expect(upload.getByText('failed', { exact: true })).toBeVisible()
    await upload.getByRole('button', { name: 'Remove spoofed.png', exact: true }).click()
    await expect(upload.getByText('spoofed.png', { exact: true })).toHaveCount(0)

    let releaseUpload: (() => void) | undefined
    const paused = new Promise<void>(resolve => { releaseUpload = resolve })
    await page.route('**/holo/panels/admin/upload', async route => {
      if (route.request().postData()?.includes('"action":"write"')) await paused
      await route.continue().catch(() => undefined)
    })
    await fileInput.setInputFiles(image)
    await expect(upload.getByRole('progressbar')).toBeVisible()
    await expect(upload.getByText('uploading', { exact: true })).toBeVisible()
    await upload.getByRole('button', { name: `Cancel ${image.name}`, exact: true }).click()
    await expect(upload.getByText(image.name, { exact: true })).toHaveCount(0)
    releaseUpload?.()
    ...
    await waitForPanelReady(page)
    await expect(upload.getByText('existing', { exact: true })).toBeVisible()
    await slugInput.focus()
    await titleInput.fill(`${title} edited`)
    await expect(slugInput).toHaveValue(`upload-lifecycle-${suffix}-edited`)
    await expect(titleInput).toBeFocused()
    ...
    await upload.getByRole('button', { name: /^Remove /u }).click()
    await expect(upload.getByText('existing', { exact: true })).toHaveCount(0)
    await fileInput.setInputFiles({ ...image, name: 'replacement.png' })
    await expect(upload.getByText('stored', { exact: true })).toBeVisible()
    await submitResourceForm(page)
    await expect(upload.getByText('existing', { exact: true })).toBeVisible()
    await submitResourceForm(page)
    ...
    await expect(slugInput).toHaveValue(`manual-edit-${suffix}`)
    await expect(upload.getByText('existing', { exact: true })).toBeVisible()
    await upload.getByRole('button', { name: /^Remove /u }).press('Enter')
    const document = await markDocument(page)
    let releaseNavigationUpload: (() => void) | undefined
    const navigationUpload = new Promise<void>(resolve => { releaseNavigationUpload = resolve })
    await page.route('**/holo/panels/admin/upload', async route => {
      if (route.request().postData()?.includes('"action":"write"')) await navigationUpload
      await route.continue().catch(() => undefined)
    })
    await fileInput.setInputFiles(image)
    await expect(upload.getByText('uploading', { exact: true })).toBeVisible()
    ...
    await expect(slugInput).toHaveValue(`session-still-active-${suffix}`)
    await fileInput.setInputFiles(image)
    await expect(upload.getByText('stored', { exact: true })).toBeVisible()
    await selectField(page, 'Category', ['Guides'])
    await selectField(page, 'City', ['Cairo'])
